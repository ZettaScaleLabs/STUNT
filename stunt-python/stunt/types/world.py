...
class World(object):
    ...
    def update(
        ...
        #     raise ValueError("planning world does not have lanes or HD map")
        self._map = hd_map
        self._lanes = lanes
        ...
        for prediction in self.obstacle_predictions:
            ...
            for transform in prediction.predicted_trajectory:
                ...
                if (
                    ...
                    > self._configuration["obstacle_filtering_distance"]
                ):
                    ...
                    if (
                        ...
                        obstacle_corners = obstacle.get_bounding_box_corners(
                            transform, self._configuration["obstacle_radius"]
                        )
    ...
    def stop_person(self, obstacle, wp_vector) -> float:
        ...
        Args:
            obstacle (:py:class:`~STUNT.prediction.obstacle_prediction.ObstaclePrediction`):  # noqa: E501
                Prediction for a person.
            wp_vector (:py:class:`~STUNT.utils.Vector2D`): vector from the ego
    ...
    def stop_for_agents(self, timestamp) -> float:
        ...
        try:
            ...
            wp_vector = self.waypoints.get_vector(
                self.ego_transform,
                self._configuration["min_pid_steer_waypoint_distance"],
            )
            ...
            # No more waypoints to follow, stopping the car
            return (0, 0, 0, 0, 0)

        for obstacle in self.obstacle_predictions:
            if obstacle.is_person() and self._configuration["stop_for_people"]:
        ...
        for obstacle in self.static_obstacles:
            if (
                ...
                and self._configuration["stop_for_traffic_lights"]
            ):
    ...
    def stop_traffic_light(self, tl, wp_vector, wp_angle) -> float:
        ...
        Args:
            tl (:py:class:`~STUNT.perception.detection.traffic_light.TrafficLight`):  # noqa: E501
                the traffic light.
            wp_vector (:py:class:`~STUNT.utils.Vector2D`): vector from the ego
                vehicle to the target waypoint
        ...
        if (
            self.ego_transform.location.distance(tl.transform.location)
            < self._configuration["traffic_light_min_distance"]
        ):
        ...
        if (
            -0.2 <= tl_angle < traffic_light_max_angle
        ...
        if (
            -0.2
            <= tl_angle
            < traffic_light_max_angle / self._configuration["coast_factor"]
            and tl_dist
