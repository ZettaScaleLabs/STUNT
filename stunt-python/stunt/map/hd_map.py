...
from stunt.types import Location, Transform
...
class HDMap(object):
    ...
    All STUNT methods should strive to use this class instead of directly
    ...
    def from_opendrive(cls, opendrive):
        return cls(CarlaMap("map", opendrive))

    def get_closest_lane_waypoint(self, location: Location) -> Transform:
        """Returns the road closest waypoint to location.

        Args:
            location (:py:class:`~STUNT.utils.Location`): Location in world
                coordinates.

        Returns:
            :py:class:`~STUNT.utils.Transform`: Transform or None if no
            waypoint is found.
        """
    ...
    def are_on_same_lane(
        ...
        Args:
            location1 (:py:class:`~STUNT.utils.Location`): Location in world
                coordinates.
            location2 (:py:class:`~STUNT.utils.Location`): Location in world
                coordinates.
        ...
        if waypoint1.road_id == waypoint2.road_id:
            ...
            if self.__is_intersection(
                waypoint1
            ) and not self.__is_intersection(waypoint2):
                return False
    ...
    def is_on_opposite_lane(self, transform: Transform):
        ...
        Args:
            transform (:py:class:`~STUNT.utils.Transform`): Transform in world
                coordinates.
    ...
    def distance_to_intersection(
        ...
        Args:
            location (:py:class:`~STUNT.utils.Location`): The starting location
    ...
    def must_obey_traffic_light(
        ...
        Args:
            ego_location (:py:class:`~STUNT.utils.Location`): Location of the
                ego vehicle in world coordinates.
            tl_location (:py:class:`~STUNT.utils.Location`): Location of the
    ...
    def get_all_lanes(self, location: Location):
        ...
        if waypoint:
            ...
            while wp_left and wp_left.lane_type == LaneType.Driving:
                left_location = Location.from_simulator(
            ...
            while wp_right and wp_right.lane_type == LaneType.Driving:
                right_location = Location.from_simulator(
    ...
    def compute_waypoints(
        ...
        Args:
            source_loc (:py:class:`~STUNT.utils.Location`): Source location in
                world coordinates.
            destination_loc (:py:class:`~STUNT.utils.Location`): Destination
                location in world coordinates.

        Returns:
            list(:py:class:`~STUNT.utils.Transform`): List of waypoint
            transforms.
        """
        ...
        return deque(
            [
                Transform.from_simulator(waypoint[0].transform)
    ...
    def _get_waypoint(
        ...
        try:
            waypoint = self._map.get_waypoint(
                location.as_simulator_location(),
